...
function ajax_post_advertise(){
    ...
    if(isset($_POST['submit'])) {
        ...
        if (empty($_POST['subcatid']) or empty($_POST['catid']) or !is_numeric(escape_html($_POST['subcatid'])) or !is_numeric(escape_html($_POST['catid']))) {
        ...
        if (empty($_POST['city']) or !is_numeric(escape_html($_POST['city']))) {
            $errors[]['message'] = $lang['CITY_REQ'];
        }
        if (!empty($_POST['price'])) {
            if (!is_numeric(escape_html($_POST['price']))) {
        ...
        if (!checkloggedin()) {
            if(isset($_POST['seller_name'])){
                $seller_name = escape_html($_POST['seller_name']);
            ...
            if(isset($_POST['seller_email'])){
                $seller_email = escape_html($_POST['seller_email']);
        ...
        if (!count($errors) > 0) {
            if (isset($_POST['item_screen']) && count(escape_html($_POST['item_screen'])) > 0) {
                $valid_formats = array("jpg", "jpeg", "png"); // Valid image formats
                $countScreen = 0;
                foreach (escape_html($_POST['item_screen']) as $name) {
        ...
        if (!count($errors) > 0) {
            ...
            if (!checkloggedin()) {
                ...
                if ($user_count > 0) {
                    ...
                    $insert_user->name = escape_html($seller_name);
            ...
            if (checkloggedin()) {
                ...
                $price = escape_html($_POST['price']);
                $phone = escape_html($_POST['phone']);
                $price = isset($_POST['price']) ? escape_html($_POST['price']) : '0';
                $phone = isset($_POST['phone']) ? escape_html($_POST['phone']) : '0';
                ...
                if($config['post_desc_editor'] == 1)
                    $description = escape_html($_POST['content']);
                else
                    $description = escape_html($_POST['content']);

                $cityid = escape_html($_POST['city']);
                ...
                if(isset($_POST['location'])){
                    $location = escape_html($_POST['location']);
                }else{
                    $location = '';
                }
                $mapLat = escape_html($_POST['latitude']);
                $mapLong = escape_html($_POST['longitude']);
                $latlong = $mapLat . "," . $mapLong;

                $post_title = removeEmailAndPhoneFromString(escape_html($_POST['title']));
                $slug = create_post_slug($post_title);

                if(isset($_POST['tags'])){
                    $tags = escape_html($_POST['tags']);
                }else{
                    $tags = '';
                }

                if(isset($_POST['videourl'])){
                    $videourl = escape_html($_POST['videourl']);
                }else{
                    $videourl = '';
                }
                ...
                $item_insrt->product_name = escape_html($post_title);
                $item_insrt->slug = $slug;
                $item_insrt->status = $status;
                $item_insrt->category = escape_html($_POST['catid']);
                $item_insrt->sub_category = escape_html($_POST['subcatid']);
                $item_insrt->description = $description;
                $item_insrt->videourl = $videourl;
                ...
                add_post_customField_data(escape_html($_POST['catid']), escape_html($_POST['subcatid']),$product_id);
                ...
                if ($amount > 0) {
                    ...
                    $_SESSION['quickad'][$access_token]['name'] = escape_html($title);
...
if(isset($_GET['country'])) {
    if ($_GET['country'] != ""){
        change_user_country(escape_html($_GET['country']));
    }
}
